...
{
	return mcp2515_init(MCP_STDEXT, canSpeed, clock);
}
...
** Descriptions:            Start transmission
*********************************************************************************************************/
...
{
    return begin(MCP_STDEXT, speedset, clockset);
}
...
INT8U MCP_CAN::readMsgBuf(INT8U *len, INT8U buf[])
{
    ...
    if (rc == CAN_OK) {
       *len = m_nDlc;
       for(int i = 0; i<m_nDlc && i < CAN_MAX_CHAR_IN_MESSAGE; i++) {
         buf[i] = m_nDta[i];
       }
    } else {
         *len = 0;
